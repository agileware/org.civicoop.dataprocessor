...
class ContactHasContributionWithStatus extends AbstractFieldFilterHandler {
  ...
  public function getConfigurationTemplateFileName() {
    return "CRM/Dataprocessor/Form/Filter/Configuration/ContactHasContributionWithStatus.tpl";
  }
  ...
  protected function getOperatorOptions(FieldSpecification $fieldSpec) {
    return array(
      'IN' => E::ts('Has contribution with status'),
      'NOT IN' => E::ts('Does not have contribution with status'),
    );
  }
  ...
  public function addToFilterForm(\CRM_Core_Form $form, $defaultFilterValue, $size='full') {
    ...
    $options = [];
    $contributionStatusApi = civicrm_api3('OptionValue', 'get', array(
    ...
    $form->add('select', "{$alias}_value", null, $options, false, [
      'style' => $minWidth,
      'class' => 'crm-select2 '.$sizeClass,
      'multiple' => TRUE,
  ...
  public function setFilter($filter) {
    ...
    if ($dataFlow && $dataFlow instanceof SqlDataFlow) {
      ...
      if (isset($filter['value']) && is_array($filter['value'])) {
        $sqlStatement .= " AND `c_{$fieldAlias}`.`contribution_status_id` IN (" . implode(",", $filter['value']) . ")";
      }
      $sqlStatement .= ")";
